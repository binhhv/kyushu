  ...
  //get average by month
	...
	$query = mysql_query($sql);
     ...
     public function getMonitoringDay($name,$day = null){
       ...
       if(is_null($day)){
          ...
          $after1days = date('Y-m-d', time() - 60 * 60 * 24 * 1);
        }
        else{
          /*$date = date_create($day);
          $currentdate = date_format($date, 'Y-m-d');
          date_modify($date, '-1 day');
          $after1days = date_format($date, 'Y-m-d');*/
		   $date = new DateTime($day);
         // $currentdate = date_format($date, 'Y-m-d');
          $currentdate = $date->format('Y-m-d');
          date_modify($date, '-1 day');
          $after1days =  $date->format('Y-m-d');//date_format($date, 'Y-m-d');

       }
   
        
	
        $sql ="SELECT * FROM `monitoringdata` WHERE (cast(day as date) = '".$after1days."' and CONVERT(hour,UNSIGNED INTEGER) >= '".$hours."' and CONVERT(hour,UNSIGNED INTEGER) <= 23) or (cast(day as date) = '".$currentdate."' and CONVERT(hour,UNSIGNED INTEGER) >= 0 and CONVERT(hour,UNSIGNED INTEGER) <= '".$hours."') and idmonitor = '".$name."' ORDER BY `day` DESC";
        $query =  mysql_query($sql);
        $arrayData = array_fill(0, 24, '');//array("","","","","","","","","","","","","","","","","","","","","","","","");
        $i = 0;
        while ($row = mysql_fetch_array($query)) {
            $index = (int)$row['hour'] - (int)$hours;
            if($index < 0){
                $index = 24 - abs((int)$row['hour'] - (int)$hours);
            }
     ...
     //2014-12-02
     public function getMonitoringDayArchive($name,$day = null){
         if(is_null($day)){
           $dt = new DateTime();
           $today = getdate();
           $hours = $today['hours'];
	...
	$query = mysql_query($sql);
     ...
     public function getMonitoringMonthYear($name,$month,$year,$type){
         if($type == 0){
             $arrayData = array_fill(1, 24, '');
             //$sql = "SELECT * FROM avgmonitoringmonth WHERE idmonitor = '".$name."' and year = ".(int)$year."";
			 //$sql = "select * from monitoringdata where DAY(day) = 1 or DAY(day) = 15 and hour = '00' and idmonitor = '".$name."'";
			  $sql = "select *, MONTH(day) as month, DAY(day) as day from monitoringdata where DAY(day) = 1 or DAY(day) = 15 and YEAR(day) = '".$year."' and hour = '00' and idmonitor = '".$name."'";
         } else {
