...
//        $this->getValue();
    ...
    function getValue(){  
        ...
        switch ($numMon){
            case 1:
                $strAddressMon = '伊都地区(1号機)';
                break;
            case 2:
                $strAddressMon = '伊都地区(2号機)';
                break;
            case 3:
                $strAddressMon = '箱崎地区(3号機)';
                break;
            default :
                $strAddressMon = '箱崎地区(4号機)';
                break;
        }
        ...
        foreach ($arrayFile as $value) {
            if (file_exists($_SERVER["DOCUMENT_ROOT"]."/data/".$value.".csv"))
            {
                ...
                $arrayValue['mon']=array('value'=>(float)$array_data[0],'unit'=>'μSv/h','fontsize'=>'','address'=>$strAddressMon);
                fclose($file); 
            }
            else {
