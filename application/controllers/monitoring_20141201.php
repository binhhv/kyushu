...
class monitoring extends CI_Controller{
    ...
    function getValue(){  
        ...
        foreach ($arrayFile as $value) {
            if (file_exists($_SERVER["DOCUMENT_ROOT"]."/data/".$value.".csv"))
            {
                ...
                $arrayValue[$this->strstr_replica($value, '_', true)]=array('value'=>$this->getRoundValue((float)$array_data[0],3,0),'unit'=>'μSv/h','fontsize'=>'');
                fclose($file); 
            }
            else {
    ...
    function getChartMonitoring($name,$day = null){
        if(is_null($day)){
             $today = new DateTime();
        }
        else {
       ...
       //getdate();
        ...
        $data['data'] = $this->MonitoringModel->getMonitoringDay($name,$day);
    ...
    public function getChartMonthYear(){
     ...
     function getChartMonitoringMonthYear($name,$month,$year,$type){
        if($type ==1){
            ...
            $arrayPoint = range(1,$daysInMonth);
    ...
    function showMonth($month, $year)
        {
            ...
            for($day = 1; $day <= $daysInMonth; $day++) {
                if( ($day + $offset - 1) % 7 == 0 && $day != 1) {
                 ...
                 $calendar .= "<td style='cursor: pointer;'>" . $day . "</td>";
            }
